		...
		<h2>Registrar informe de turno</h2>
          <div class="col-md-10">
				...
				<br>
                 ...
                 <?php 
                  ...
                  <select required class="form-control" id="selfechas" name="selfechas">
					...
					$idfecha = $rowfechas["id"]; 
					$id = $rowfechas["nrofecha"]; 
			...
			<?php if($_POST['send']=="Enviar"){
				...
				$sqlconfirmar="select fecha from informe where fecha=$fechas";
				$rsconfirmar=mysql_query($sqlconfirmar,$cnn);
				$rowconfirmar=mysql_num_rows($rsconfirmar);
				
					//confirmar que informe de fecha ya se haya agregado
					
					
				
				if($rowconfirmar!=0){echo "<script>alert('Informe de fecha ya existe')</script>";}else{		
				$sql="insert into informe values ('','$turno', '$fechas', '$informe', '1')";
